...
function createStoredProcedures() {
	...
	sql =
		...
		")\n" +
		"BEGIN\n" +
		"SELECT * FROM transfer WHERE transfer.to_account = AccountNumber;\n" +
		"END;";
	serverConnection.query(sql, (error, results, fields) => {
		if (error) {
			console.log(error.message);
...
function GetAllUserData() {
	let sql = "SELECT user_id, first_name, last_name, role_id FROM user WHERE user.role_id IN(2,3)";
	return new Promise((resolve, reject) => {
		serverConnection.query(sql, (error, results, fields) => {
			if (error) {
				console.log(error.message);
...
function CompareTransferDates(a, b) {
	return a.transfer_date > b.transfer_date ? 1 : a.transfer_date < b.transfer_date ? -1 : 0;
}
